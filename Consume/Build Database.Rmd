...
#### Remote IO Locations
```{r remote-io}
F_INPUT_DIR = "M:/Data/Traffic/Tableau Summaries/_working/"
INPUT_FILE  = "98_13_PMwithdecimal_no_single.txt"

F_OUTPUT_DIR = "M:/Data/Traffic/Tableau Summaries/_working/"
...
#### End timer
```{r timer-end}
run_time <- start_time - proc.time()
run_time
```

```{r debug}
karate <- typical_sum_df %>%
  filter(days_observed > 80)

table(karate$station)

kungfu <- typical_df %>%
  filter(station == 76) %>%
  filter(date == '2000-10-19') %>%
  filter(direction == 'E')
...
# START HERE
# Sum over these plus integer hour with typical and flag duplicate records
# Remove in clean up step
